...
# then turn it into 2D windows and save that as a compressed .npz file
# this dataset will be what time-series models train on
...
def make_windows(run_df, faultNum, runNum, window_size, step_size, fault_start_sample_threshold=None):
    ...
    # The run is already sorted by sample before this function is called.
    # Pull the sensor values and sample numbers into NumPy arrays once,
    # then slice those arrays directly for each window.
    sensor_values_np = run_df[SENSOR_COLS].to_numpy(dtype=np.float32)
    sample_values_np = run_df[SAMPLE_COL].to_numpy()
    ...
    while start_of_window + window_size <= samples_count:
        ...
        window = sensor_values_np[start_of_window:end_of_window]

        # Use the sample value at the end of the window to decide
        # whether this window is still pre-fault normal data.
        window_end_sample = int(sample_values_np[end_of_window - 1])
...
def main():
    ...
    output_filename = f"small_fault{first_fault}_{last_fault}_runs{first_run}_{last_run}_W{window_size}_step{step_size}_2d.npz"
    ...
    all_inputs = []         # list of 2D windows (each shape window_size x 52)
    ...
    for fault in faults_to_use:
        ...
        for simRun in runs_to_use:
            ...
            # Keep windows in 2D form for time-series models:
            # each input is shape (window_size, 52)
            inputs = windows
            answers = faults
    ...
    # inputs shape becomes:
    # (num_windows, window_size, 52)
    ...
    print("\nDone building.")
    print("Total number of windows:", len(inputs_np_array))
    ...
    # save everything
    # np.savez is faster than np.savez_compressed, but creates a larger file.
    np.savez(
        file=output_path,
